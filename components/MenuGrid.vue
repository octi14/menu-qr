  ...
  <div class="min-h-screen transition-all duration-300 ease-in-out" :style="{ backgroundColor, fontFamily, color: textColor, '--price-color': priceColor }">
    <div class="mx-auto flex min-h-screen max-w-6xl flex-col gap-8 px-5 py-10 sm:px-6">
        ...
        <div class="flex gap-2 overflow-x-auto scrollbar-hide pb-2">
          ...
          >
            <span class="inline-flex items-center gap-1">
              ...
              <span
                v-if="editable && isPreview && selectedCategory === section.id && !isEditing('section', section.id, 'name')"
                class="inline-flex items-center justify-center opacity-85 transition-opacity hover:opacity-100"
                :style="deleteIconStyle"
                title="Eliminar sección"
                aria-label="Eliminar sección"
                role="button"
                @click.stop="openDeleteSectionConfirm(section.id)"
              >
                <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <main class="flex-1" role="main">
        <div v-if="editable && isPreview && selectedCategory" class="mb-3 flex items-center justify-end">
          <button
            type="button"
            class="inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors"
            :style="{ borderColor: priceColor, color: priceColor, backgroundColor: `${priceColor}10` }"
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
          ...
          >
            <!-- Imagen del ítem (o placeholder comida/bebida) -->
            <div class="aspect-square overflow-hidden bg-slate-100">
              <img
                :src="getMenuItemImageSrc(item, item.sectionName)"
                :alt="item.name"
                loading="lazy"
            ...
            <div class="p-3 space-y-2">
              ...
              >
                ...
                <input
                  v-else
                  v-model="item.name"
                  class="w-full rounded border border-slate-300 bg-white/90 px-1.5 py-0.5 text-xs text-slate-900"
                  @blur="stopEditing"
                  @keydown.enter.prevent="stopEditing"
                />
              </h3>
              <p v-if="item.description && !isEditing('item', item.id, 'description')" class="group text-xs opacity-70 line-clamp-2">
                <span class="inline-flex items-center gap-1">
                  {{ item.description }}
                  <button
                    v-if="editable"
              ...
              <textarea
                ...
                rows="2"
                class="w-full resize-none rounded border border-slate-300 bg-white/90 px-1.5 py-0.5 text-xs text-slate-900"
                @blur="stopEditing"
              />
              <div class="flex items-center justify-between gap-2">
                <button
                  ...
                  type="button"
                  class="absolute right-2 top-2 p-0.5 opacity-0 transition-opacity group-hover:opacity-100"
                  :style="deleteIconStyle"
                  title="Eliminar ítem"
                ...
                <span
                  ...
                  class="group inline-flex items-center gap-1 text-base font-bold tabular-nums"
                ...
                <input
                  ...
                  min="0"
                  step="0.01"
                  class="w-20 rounded border border-slate-300 bg-white/90 px-1.5 py-1 text-xs text-slate-900"
                  @blur="stopEditing"
                  @keydown.enter.prevent="stopEditing"
                />
                <span
...
const allItems = computed(() => {
  ...
  sections.value.forEach(section => {
    section.items.forEach(item => {
      // Importante: mantener la referencia real del item para que los cambios
      // inline persistan en props.business.sections y se guarden al hacer Save.
...
const openDeleteSectionConfirm = (sectionId) => {
...
const openDeleteItemConfirm = (item) => {
  ...
  deleteTargetSectionId.value = item?.sectionId || ''
